...
|------|--------|------------|---------|
| 写入 | **7µs** | ~200ms | ~1ms |
| 搜索（top-10） | **132µs** | ~300ms | ~10ms |
| 上下文生成 | **51µs** | ~500ms | 手动拼接 |
| 信念更新 | **27µs** | 不支持 | 不支持 |
| 社交图谱 | **13µs** | 付费版 | 不支持 |
| 1K 条记忆搜索 | **1.2ms** | ~500ms | ~50ms |

比 Mem0 云端**快 2,266 倍**。并且提供 Mem0 和文件方案都没有的功能。
...
- **v0.5** — 对话压缩，关系推理，多模态记忆
- **v1.0** — 跨设备同步（CRDT，无需云端），插件系统
